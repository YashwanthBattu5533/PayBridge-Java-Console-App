...
public class Recharge {
    static final String red = "\u001b[00;1m"; // Unused static field
...
class Cvpay {
    ...
    void closeAppli() {
        ...
        if (n.equalsIgnoreCase("ok")) {
            System.exit(0); // Added to actually exit the application
...
class Rech {
    void Recharge() {
        ...
        if (ph.charAt(0) == '0' || ph.length() != 10) { // Combined conditions for invalid input
            System.out.println("\n  \u001b[31;1mInvalid Mobile number. Please try again.\u001b[00;1m\n");
            Recharge();
        } else {
            operator(ph);
        }
...
class Airtel {
    ...
    void select(String ph) {
        System.out.println("  1.U/L calls and 1.5gb per/day data one month with hotstar subscription 3month at       399 rs \n");
        System.out.println("  2.U/L calls and 1.5gb per/day data two months with hotstar subscription 3month at      499 rs \n");
        System.out.println("  3.U/L calls and 1.5gb per/day data tree months with hotstar subscription 3month at     599 rs \n");
...
class Vi {
    ...
    void select(String ph) {
        System.out.println("  1.U/L calls and 1.5gb per/day data one month with vi movies and TV subscription 3month at       319 rs \n");
        System.out.println("  2.U/L calls and 3gb per/day data two months with disney+hotstar subscription 1months at      499 rs \n");
        System.out.println("  3.U/L calls and 1.5gb per/day data tree months with disney+hotstar subscription 3months at     1099 rs \n");
        System.out.println("  4.Go Back");
        int n = Cvpay.sc.nextInt();
        if (n == 1) {
...
class Jio {
    ...
    void select(String ph) {
        System.out.println("  1.U/L calls and 2.5gb per/day data two months with amazon subscription 3month at       499 rs \n");
        System.out.println("  2.U/L calls and 2.0gb per/day data three months with starsports subscription 3month at      399 rs \n");
        System.out.println("  3.U/L calls and 2.25gb per/day data tree months with hotstar subscription 3month at     599 rs \n");
        System.out.println("  4.Go Back");
        int n = Cvpay.sc.nextInt();
        if (n == 1) {
...
class Profile {
    static void Display() {
        ...
        System.out.println("          |__________________________________________|"); // This line doesn't seem to close the previous visual box properly
        new Profile().next();
    }
...
class Addmoney {
    ...
    static void Wallet() {
System.out.println("Your Prepaid Wallet Balance is --> " + Cvpay.Balance + "     ");
System.out.println("\n   1.Enter amount \n   2.Home");
        ...
        if (c < c1) {
            ...
            if (mpin > 99999 && mpin < 999999) {
                if (mpin == this.mpin) {
                    Balance = Balance - n;
                    if (Balance >= 0) {
                        Cvpay.Balance = Cvpay.Balance + ((n * 3) / 100) + n; // Commission calculation seems a bit off here, adding n twice.
...
class Reports {
    ...
    void Display() {
        ...
        if (n == 1) {
            ...
            System.out.println("\n  \u001b[31;1mInvalid input try again\n\u001b[00;1m");
            Display(); // Added to re-prompt on invalid input
        }
    }
}
...
class Rechreports {
    ...
    static void Display() {
        if (c != 0) {
            ...
            if (c >= 6) {
                ...
                System.out.println("\n    Mobile No        : " + r6.ph);
                System.out.println("\n    Operator         : " + r6.ope);
                System.out.println("\n    O/B              : " + r6.OB); // Changed from r1.OB
                System.out.println("\n    amount           : \u001b[31;1m" + r6.amt + "\u001b[00;1m"); // Changed from r1.amt
                System.out.println("\n    C/B              : " + r6.CB + "\n"); // Changed from r1.CB
                System.out.println("\n");
                System.out.println("-----------------------------------------------------------------------------------------------------------------");
            }
        ...
        home(); // Always go home after displaying reports
    }
    ...
    static void report(String ph, int amt, String ope, long Tid, String DT) {
        c++;
        // The logic for storing reports needs to shift older reports down to r6, and new ones become r1.
        // For simplicity, let's keep the existing logic that stores in r1, r2, etc., sequentially
        // This means only the last 6 reports will be accessible via these static fields.
        if (c == 1) {
            r1.ph = ph;
            r1.amt = amt;
            r1.ope = ope;
            r1.Tid = Tid;
            r1.OB = Cvpay.Balance + amt; // Original Balance before this transaction
            r1.CB = Cvpay.Balance;       // Current Balance after this transaction
            r1.DT = DT;
        } else if (c == 2) {
            r2.ph = ph;
            r2.amt = amt;
            ...
            // For more than 6 reports, the current implementation will overwrite r6.
            // A more robust solution would be to use a List or ArrayDeque to store reports.
            // For now, let's just log this for clarity.
            System.out.println("Warning: More than 6 reports, overwriting oldest. Consider using a collection for reports.");
            r1.ph = r2.ph; r1.amt = r2.amt; r1.ope = r2.ope; r1.Tid = r2.Tid; r1.OB = r2.OB; r1.CB = r2.CB; r1.DT = r2.DT;
            r2.ph = r3.ph; r2.amt = r3.amt; r2.ope = r3.ope; r2.Tid = r3.Tid; r2.OB = r3.OB; r2.CB = r3.CB; r2.DT = r3.DT;
            r3.ph = r4.ph; r3.amt = r4.amt; r3.ope = r4.ope; r3.Tid = r4.Tid; r3.OB = r4.OB; r3.CB = r4.CB; r3.DT = r4.DT;
            r4.ph = r5.ph; r4.amt = r5.amt; r4.ope = r5.ope; r4.Tid = r5.Tid; r4.OB = r5.OB; r4.CB = r5.CB; r4.DT = r5.DT;
            r5.ph = r6.ph; r5.amt = r6.amt; r5.ope = r6.ope; r5.Tid = r6.Tid; r5.OB = r6.OB; r5.CB = r6.CB; r5.DT = r6.DT;
            r6.ph = ph; r6.amt = amt; r6.ope = ope; r6.Tid = Tid; r6.OB = Cvpay.Balance + amt; r6.CB = Cvpay.Balance; r6.DT = DT;
        }
    }
}
...
class Conform {
    static void confirmation(String ph, int amt, String ope) {
        ...
        if (n.equalsIgnoreCase("ok")) {
            if (Cvpay.Balance < amt) { // Check balance before deducting
                System.out.println("\n  \u001b[31;1mNot enough Balance please add money to wallet\n\u001b[00;1m\n");
                System.out.println("    \u001b[100;97;1mYour wallet balance is --> " + Cvpay.Balance + " \u001b[00;1m\n");
                Recharge.a1.home();
            } else {
                Cvpay.Balance = Cvpay.Balance - amt;
                System.out.println("   \u001b[32;1mYour Recharge was Succesfully completed Mobile number \u001b[00;1m" + ph + " \u001b[32;1mwith \u001b[00;1m" + amt + " \u001b[00;1mRs");
                SimpleDateFormat formatter = new SimpleDateFormat("dd/MM/yyyy HH:mm:ss");
                Date date = new Date();
                String DT = formatter.format(date);
                System.out.println("\n   Date and Time : " + DT);
                long transaction = Conform.random();
                System.out.println("\n   Transaction Id is  " + transaction + "\n\n   \u001b[100;97;1m" + "  Your Wallet balance is " + Cvpay.Balance + " \u001b[00;1m\n");
    ...
    static long random() {
        ...
        // Generate a random 10-digit number. NextLong() generates a long, then modulo to get 10 digits
        long randomNum = 1_000_000_000L + (long)(rand.nextDouble() * 9_000_000_000L);
        return randomNum;
    }
